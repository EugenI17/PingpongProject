...
public class GameService {
   ...
   public void createGame(GameDto gameDto,Long player1Id,Long player2Id) {
      ...
      {
           gameDto.setWinner(MainRepository.findById(player1Id).get().getName());
      }
       else
      {
           gameDto.setWinner(MainRepository.findById(player2Id).get().getName());
      }
   ...
   public String WinnerId(String winner){
        List<Player> players = MainRepository.findAll();
   ...
   public List<GameDto> getAllGames()
    ...
    public void SetWins(Long id){
        ...
        if(game.getWinner().equals(MainRepository.findById(game.getPlayer1Id()).get().getName())){
            MainRepository.findById(game.getPlayer1Id()).get().setWins(MainRepository.findById(game.getPlayer1Id()).get().getWins()-1);
        }
        else{
            MainRepository.findById(game.getPlayer2Id()).get().setWins(MainRepository.findById(game.getPlayer2Id()).get().getWins()-1);
        }
